...
function outputFormation(question) {
  ...
  var notFineTrigger = localStorage.getItem('notFineTrigger');
  var notDictTrigger = localStorage.getItem('notDictTrigger');
  ...
  */
  ...
  if (notFineTrigger == "true") {
    var answer = "okk am sorry";
    localStorage.removeItem('notFineTrigger');
  }
  ...
  */
  ...
  if (question.match('.*\\bfine\\b.*') || question.match('.*\\bgood\\b.*') || question.match('.*\\bwell\\b.*')) {
    if (fineTrigger == 'true') {
      if (question.match('.*\\bnot\\b.*')) {
        var notFineTrigger = "true";
        localStorage.setItem('notFineTrigger', notFineTrigger);
        var msg1 = new SpeechSynthesisUtterance('Why? Whats the problem');
        window.speechSynthesis.speak(msg1);
        answer = "Why?, Whats the problem";
        localStorage.removeItem('fineTrigger');
      }
      else if (question.match('^(?!.*not).*$')) {
        var msg2 = new SpeechSynthesisUtterance('That is great!');
  ...
  */
  ...
  if (question.match('.*\\bsave note as\\b.*')) {
    ...
    if (noteTitleTemp != null) {
      if ((savedNote1 == null) && (changeTitle1 == null)) {
        titleInput.value = noteTitleTemp;
  ...
  if (question.toUpperCase() == 'SHOW ME ALL NOTES' || question.toUpperCase() == 'SHOW ALL NOTES' || question.toUpperCase() == 'ALL NOTES' || question.toUpperCase() == 'SHOW NOTES') {
      if (ntErased != "true") {
        ...
        localStorage.removeItem('ntErased');
      }
      else {
  ...
  */
  ...
  if (question.match('.*\\bif user types\\b.*')) {
    var userString = question.slice(question.indexOf('types') + 6, question.lastIndexOf('then') -1);
    var userString = '\'' + userString + '\'';
    var commandString = question.substring(question.indexOf('then') + 5);
  ...
  //variable from 'edit script' interaction
  eval(addedScript);
  //variable from non-coding learning interaction
  eval(addedScriptSC);
  ...
  */
  ...
  if (question.match('.*\\bdefine\\b.*')) {
    var askedWord = question.substring(7);
    var dictTrigger = "true";
    localStorage.setItem('askedWord', askedWord);
    if (word != null) {
      answer = word;
      localStorage.removeItem('word');
      localStorage.removeItem('askedWord');
    } else {
      answer = 'Definition is loading, try again.';
    }
    if (dictTrigger == "true") {
      function getWord() {
        var askedWord = localStorage.getItem('askedWord');
        $.getJSON('https://raw.githubusercontent.com/adambom/dictionary/master/dictionary.json', function(data) {
          eval("var word = data['" + askedWord.toUpperCase() + "'];");
          if (word != null) {
            localStorage.setItem('word', word);
          } else {
            /*
            if the word doesnt match with any in the dictionary then yveete will google it 
            However you need a trigger to redirect the if/else condition out of the getWord()
            function or else a new tab will open every 2000 milliseconds
            */
            var notDictTrigger = 'true';
            localStorage.setItem('notDictTrigger', notDictTrigger);
          }
        });
    ...
    if (notDictTrigger == "true") {
      window.open('https://www.google.com/search?q=define%20' + askedWord);
      answer = 'The definition of the word you were looking for has been opened on a new window';
      /* 
        the trigger needs to be removed or else google will always open each time the user
